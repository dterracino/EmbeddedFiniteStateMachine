...
#include "efsmDebugTester.h"
		...
		EFSM_DEBUG_CONTROL efsmDebugControl;
			...
			char debugStatusTxFilename[EFSM_DEBUG_INTERFACE_FILES_FILENAME_BUFFER_SIZE];
			char debugCommandRxFilename[EFSM_DEBUG_INTERFACE_FILES_FILENAME_BUFFER_SIZE];
		#endif
	#endif
#endif
...
void FlushFileContents(char * filename)
{
...
{
#if(EFSM_CONFIG_DEBUG_MODE == EFSM_DEBUG_MODE_DESKTOP)
	ReadFileToBuffer(efsmDebugControl.debugBuffer, EFSM_DEBUG_BUFFER_SIZE, EFSM_DEBUG_INTERFACE_EMBEDDED_COMMAND_RECEIVE_FILE);	
#endif
}
...
{
#if(EFSM_CONFIG_DEBUG_MODE == EFSM_DEBUG_MODE_DESKTOP)
	WriteBufferToFile(efsmDebugControl.debugBuffer, efsmDebugControl.debugBufferFrameSize, EFSM_DEBUG_INTERFACE_EMBEDDED_STATUS_TRANSMIT_FILE);
#endif
}
...
{
	if (!efsmDebugControl.initialized)
	{
		EFSM_InitializeProcess();
		efsmDebugControl.initialized = 1;
	}
	...
	if (efsmDebugControl.postingStatus == EFSM_DEBUG_PROTOCOL_POSTING_STATUS_MASTER_COMMAND_CYCLE)
	{
		...
		FlushFileContents(EFSM_DEBUG_INTERFACE_EMBEDDED_COMMAND_RECEIVE_FILE);
